...
GetOptions (
    ...
    "key=s"  => \$key_files,
    "binary" => \$gen_binary,
    "key_list=s" => \$key_list_file
)    or die("gen_rsa_pattern  --msg [msg_file (binary)] [--key_list] --key \"openssl key text files\"  [--binary] [--noreverse] [--wordswap]\n");

if($key_list_file ne '')
{
    open (LIST, "<$key_list_file") or die "Fail to open key list file $key_list_file\n";

    while (<LIST>){
        $_ =~ s/\s//g;
        if($_ ne '')
        {
         push (@key_file_list, $_);
    print "//DEBUG: key reg $_\n";
     }
     }

    close(LIST);
    print "//DEBUG: use key files @key_file_list\n";
}
else
{
@key_file_list = split(' ', $key_files);
}
my $tmp = @key_file_list;
print("const unsigned int RSA_KEY_COUNT = $tmp;\n");

foreach my $key_file (@key_file_list)
{
    $mod_hex ='';
    $pub_exp_hex = '';
    $pri_exp_hex = '';
    ...
    ##Message Data ...
    if($msg_file eq '' || $msg_file eq 'temp_msg.dat')
    {
        print "//Using random message\n";
        $msg_file = 'temp_msg.dat';
        open(my $MSG, '>:raw', $msg_file) or die "Fail to open message file $msg_file\n";
        
        my $i=0;
        my $random_number=0;
        for (; $i< $KEY_BITS/8; $i++)
        {
            #first byte is '00' for "Data should be larger than modulus constraints"
            print $MSG pack('C', $random_number);
            $msg_hex .= sprintf("%02x", $random_number);
            $random_number = int(rand(256));
        }
        close($MSG);
    }
    else
...
#padding to  mudulus significant bits
    ...
    if($PARSE_MODE eq 'from_openssl')
    {
        my $cmd = "\.\\OpenSSL-Win32\\bin\\openssl.exe rsautl -encrypt -inkey $key_file -raw -in $msg_file > enc.dat";
        qx($cmd);
        print ("//[openssl] $cmd");
...
print ("//first byte in messgae is ZERO for \"Data can not larger than modulus constraints\"\n");
